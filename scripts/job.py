...
volumes_df = raw_volumes_df.select(
    col("id").cast("bigint").alias("volume_id"),
    col("name"),
    col("publisher.id").cast("bigint").alias("publisher_id"),
    col("publisher.name").alias("publisher_name"),
    col("start_year").cast("int").alias("start_year"),
    col("count_of_issues").cast("int").alias("count_of_issues"),
    col("first_issue.id").cast("bigint").alias("first_issue_id"),
    col("last_issue.id").cast("bigint").alias("last_issue_id"),
    col("api_detail_url"),
    col("site_detail_url"),
    col("image.medium_url").alias("image_url"),
    col("description"),
    to_timestamp(col("date_added")).alias("date_added"),
    to_timestamp(col("date_last_updated")).alias("date_last_updated"),
...
details_df = raw_details_df.select(
    col("raw_detail.id").cast("bigint").alias("issue_id"),
    col("raw_detail.volume.id").cast("bigint").alias("volume_id"),
    col("raw_detail.volume.name").alias("volume_name"),
    col("raw_detail.issue_number").alias("issue_number"),
    col("raw_detail.name").alias("name"),
    col("raw_detail.description").alias("description"),
    col("raw_detail.image.medium_url").alias("image_url"),

    expr("transform(raw_detail.character_credits, x -> x.name)").alias("characters"),
    expr("transform(raw_detail.object_credits, x -> x.name)").alias("objects"),
    expr("transform(filter(raw_detail.person_credits, x -> lower(x.role) like '%writer%'), x -> x.name)").alias("writers"),
    expr("transform(filter(raw_detail.person_credits, x -> lower(x.role) like '%artist%'), x -> x.name)").alias("artists"),
    expr("transform(filter(raw_detail.person_credits, x -> lower(x.role) like '%colorist%'), x -> x.name)").alias("colorists"),
    expr("transform(raw_detail.team_credits, x -> x.name)").alias("teams"),
    expr("transform(raw_detail.location_credits, x -> x.name)").alias("locations"),
    expr("transform(raw_detail.concept_credits, x -> x.name)").alias("concepts"),

    col("raw_detail.site_detail_url").alias("site_detail_url"),
    col("raw_detail.api_detail_url").alias("api_detail_url"),
    to_date(col("raw_detail.store_date")).alias("store_date"),
    to_date(col("raw_detail.cover_date")).alias("cover_date"),
    to_timestamp(col("raw_detail.date_added")).alias("date_added"),
    to_timestamp(col("raw_detail.date_last_updated")).alias("date_last_updated"),
    to_timestamp(col("fetched_at")).alias("detail_fetched_at"),
    to_date(lit(ingestion_date)).alias("ingestion_date"),
    lit(run_id).alias("run_id")
)
